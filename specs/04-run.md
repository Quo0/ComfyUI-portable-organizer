...
| ID | Requirement |
|---|---|
| `FR-RUN-010` | An instance launches with the chosen profile; the last profile used is remembered |
| `FR-RUN-020` | When launched from the app, no browser opens |
| `FR-RUN-030` | The app hands the instance a free port, respecting the preferred one |
| `FR-RUN-040` | The startup logs are visible in real time |
| `FR-RUN-050` | The app determines the moment the server becomes ready |
| `FR-RUN-060` | An instance's state is reflected in the interface and distinguishes ordinary outcomes from crashes |
| `FR-RUN-070` | Stopping terminates the server and releases the port |
| `FR-RUN-080` | Quitting the app leaves no servers running |
| `FR-RUN-090` | The user is warned about the risks of running several instances at once |
| `FR-RUN-100` | A server restart performed by ComfyUI-Manager is recognised rather than looking like a crash |
| `FR-RUN-110` | The launch arguments can be viewed and changed |
| `FR-RUN-120` | The app does not appear as a console window on top of the interface |
